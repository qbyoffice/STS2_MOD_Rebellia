...
namespace Rebellia.RebelliaCode.Cards.Ancient
{
    public class PrimordialCrimsonRaid : RebelliaCard
    {
        ...
        protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay play)
        {
            ...
            int currentPoints = bloodPower.GetPoints();
            if (currentPoints <= 0)
                return;

            bool consumed = await Utils.TryConsumeBloodArtPoints(Owner.Creature, currentPoints);
            if (!consumed)
                return;

            var drawPile = PileType.Draw.GetPile(Owner);
            var availableCards = drawPile.Cards.ToList();
            int selectCount = Math.Min(currentPoints, availableCards.Count);
            if (selectCount == 0)
                return;
